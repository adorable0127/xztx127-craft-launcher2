...
public partial class DownloadCenterPage : UserControl
{
    ...
    private async void InstallVersion_Click(object sender, RoutedEventArgs e)
    {
        ...
        // 需求：点「下载安装」之后要有一个立刻能看到的反馈，否则用户点完发现界面毫无变化，
        // 会以为自己没点中/下载根本没开始，转头又点了一次甚至跑去下载中心之外的地方找入口。
        // 不用重新弹一个会挡住主界面的进度弹窗/页面跳转——那正是上面 StartNew 那段注释里
        // 特意去掉的旧体验（挡住主界面、不能同时启动游戏）。用右下角 Toast 给一次性的
        // "已经开始了"确认，跟"游戏已启动""下载完成"这些反馈是同一套语言，用户已经熟悉，
        // 后续进度看标题栏下载列表即可，不需要为了这一条反馈重新引入被淘汰掉的模态进度页。
        ToastService.ShowInfo($"已开始下载「{entry.Id}」，可在标题栏下载列表查看进度");
    ...
    ///
    /// 二次修复（"社区资源自动选择的文件夹不完整"）：这个方法之前返回的是"版本根目录"
    /// （.../versions/&lt;id&gt;），**不包含**最后那层 "mods" 子目录，指望调用方后续下载时
    /// 再拼接一层。但 DownloadModInlineAsync 为了支持"用户可以在弹出的资源管理器里任选
    /// 一个自定义目录、所见即所得"，统一传的是 appendCategorySubdir:false（表示"调用方给的
    /// 就是最终落点，不要再自动拼子目录"）。两者一叠加：只要用户直接确认默认建议（没有手动
    /// 换目录），mod jar 就会被直接扔进"版本根目录"本身，而不是它下面的 mods/ 子目录——
    /// 文件夹路径"看起来选对了"（确实是当前版本的目录），但缺了最后一层，游戏的 mod
    /// 加载器根本不会去版本根目录扫描 jar，表现跟老 bug 一样：下载成功、游戏里却看不到。
    /// 现在直接在这里把 "mods" 拼好、返回完整路径，PromptSaveDirectory 弹窗里给用户看到的
    /// 默认建议就是这个已经补全的完整路径；用户如果选"选择其他目录"另外挑了自定义位置，
    /// 那个自定义位置依然按"所见即所得"原样使用，不受影响。
    /// </summary>
    private string GetEffectiveModDir(string folderPath, out bool hasSelectedVersion)
    {
        ...
        var versionRoot = hasSelectedVersion ? Path.Combine(folderPath, "versions", versionId!) : folderPath;
        return Path.Combine(versionRoot, "mods");
    }
    ...
    private async Task DownloadResourceInlineAsync(UnifiedResourceItem item, InlineVersionEntry entry)
    {
        ...
        // 需求："修复社区资源不会走右上角全局下载管理"。
        // 之前这里用一个独占的 ProgressDialog 模态弹窗，跟标题栏的下载列表完全是两套体系：
        // 用户下载一个材质包/数据包，界面就被这个弹窗焊死，既看不到进度汇总，也没法切到
        // 别的页面继续操作，跟"游戏版本"下载（走 DownloadQueueService.StartNew）体验完全割裂。
        // 现在改成跟"游戏版本"下载同一条路：登记进 DownloadQueueService，标题栏下载列表
        // 直接就能看到这一条，支持切页面/暂停/取消，"开始下载"的气泡提示（StartNew 内部统一
        // 处理）和"下载完成"提示（ShowCompletionNotification，按 "下载 " 前缀识别成社区资源）
        // 也都自动生效，不需要在这里各自再弹一次。
        //
        // 资源类型（材质包/光影包/数据包）在下载真正开始前先固定成局部变量：resumeAction 是
        // 异步执行的，_currentResourceType 是页面级字段，用户下载排队期间完全可能已经切到了
        // 别的分类 Tab，字段值会变；固定成局部变量能保证这一条任务从头到尾用的是发起下载那一刻
        // 选中的类型，不会被后续的 Tab 切换污染。
        var resourceType = _currentResourceType;
        var isDataPack = item.IsDataPack;
        var selectedSaveName = item.SelectedSaveName;

        DownloadQueueService.Instance.StartNew($"下载 {entry.Name}", async (queueItem, ct) =>
        {
            var progress = new Progress<string>(msg => queueItem.StatusText = msg);
            string path;
            if (entry.Source == ModSource.Modrinth)
            {
                ...
                //
                // 修复：这里之前传的是 `!item.IsDataPack`——数据包场景下 item.IsDataPack==true，
                // 算出来的是 appendCategorySubdir=false，跟上面这行注释的意图完全相反！
                // DownloadResourceAsync 内部对 appendCategorySubdir=false 是最高优先级分支
                // （`if (!appendCategorySubdir) destDir = minecraftDir;`），会直接跳过按类型分流的
                // "saves/<存档名>/datapacks" 拼接逻辑，saveName 参数因此完全没被用上。
                // 实际后果：数据包被原样扔进 effectiveDir（也就是 folder.Path，.minecraft 根目录）
                // 里，而不是进了存档的 datapacks/ 文件夹——游戏读取数据包只认存档目录下的
                // datapacks/，根目录这个文件对游戏来说形同不存在，用户看到"下载成功"提示，
                // 打开游戏却发现数据包没生效。材质包/光影包不受影响，因为它们从不进入这个分支
                // （效果目录已经在上面 PromptSaveDirectory 里被用户选择/确认过，这里统一
                // 恒为 true 才能让 DownloadResourceAsync 按类型正确拼接落地子目录）。
                path = await _modrinth.DownloadResourceAsync(effectiveDir, resourceType,
                    (ModrinthVersion)entry.RawVersion, progress, isDataPack ? selectedSaveName : null,
                    appendCategorySubdir: true, ct: ct);
            }
            else
            {
                var kind = resourceType switch
                {
                ...
                // 同上：CurseForgeService.DownloadResourceAsync 内部结构跟 Modrinth 那份完全一致，
                // 同样的 `!item.IsDataPack` 反了会导致同样的"数据包落进 .minecraft 根目录"问题。
                path = await GetCurseForge().DownloadResourceAsync(effectiveDir, kind,
                    (CurseForgeFile)entry.RawVersion, progress, isDataPack ? selectedSaveName : null,
                    appendCategorySubdir: true, ct: ct);
            }
    ...
    private async Task DownloadModpackInlineAsync(UnifiedResourceItem item, InlineVersionEntry entry)
    {
        ...
        // 同 DownloadResourceInlineAsync：接入全局下载队列。名字用"整合包 "前缀，
        // 跟 DownloadQueueService.ShowCompletionNotification 里已经在识别的
        // `item.Name.StartsWith("整合包 ")` 分支对上，"下载完成不弹窗"这个设置项才会对
        // 整合包这类任务真正生效（之前整合包走独立 ProgressDialog，完全没进过这个判断，
        // 那个开关形同虚设——即使用户在设置里关掉了整合包完成提示，装完整合包也照样弹）。
        DownloadQueueService.Instance.StartNew($"整合包 {item.Title}", async (queueItem, ct) =>
        {
            ...
            try
            {
                ...
                queueItem.StatusText = $"下载整合包清单 {file.Filename}";
                using (var http = new HttpClient())
                using (var resp = await http.GetAsync(file.Url, ct))
                {
                ...
                var progress = new Progress<string>(msg => queueItem.StatusText = msg);
                var result = await _modpackService.ImportMrpackAsync(tempMrpackPath, targetVersionDir, progress, ct);
