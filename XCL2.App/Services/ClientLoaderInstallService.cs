...
public class ClientLoaderInstallService : IDisposable
{
        ...
        IProgress<ProgressInfo>? progress, CancellationToken ct = default, string? customInstanceName = null)
    {
        ...
        // 修复"下载版本不会下载 jar 文件，造成无法开始启动步骤"：这一步是把原版 client.jar 复制成
        // 这个加载器实例自己的 jar，前面已经把 profile json 的 InheritsFrom 清空了（做成独立实例），
        // 一旦这一步因为任何原因找不到父版本 jar（比如父版本本身没装完整、被用户手动删过部分文件），
        // 之前的代码是直接跳过、不报错，最终产物是一个只有 json、没有 jar 又没有 InheritsFrom 兜底的
        // "空壳"版本，进度还是照常汇报"安装完成"，用户直到点启动才会发现根本起不来。改成找不到就
        // 直接抛异常中止安装，不生成这种半成品，跟下面 OptiFine 安装那处已有的同类检查保持一致。
        var parentJarPath = ResolveVersionFile(parentVersionDir, mcVersion, "jar")
            ?? throw new InvalidOperationException(
                $"原版父版本 {mcVersion} 已安装但找不到 {mcVersion}.jar，无法继续安装当前加载器（父版本可能没有安装完整，建议先删除后重新安装该原版版本）。");
