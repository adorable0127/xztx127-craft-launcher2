...
public static class Win11EffectsService
{
    ...
    /// <summary>
    /// AppConfig.EnableWinUi3Design 对应的当前状态。修复"设置页里能勾选 WinUI 3 新设计，
    /// 保存重启后界面却看不出任何变化"：这个开关之前只在 SettingsPage 里读写 cfg 字段，
    /// 从来没有任何代码读取过它、也没有接到任何实际视觉效果上——纯粹是个不生效的摆设。
    /// 现在跟 CurrentEnabled（Win11 新光效）各自独立地控制窗口圆角：只要这两个开关任意一个
    /// 开着，窗口就应该有 Win11 风格的圆角——圆角本身不依赖 Mica/Acrylic 背景材质，是 WinUI 3/
    /// Fluent 设计语言的基础特征之一，就算用户没勾"新光效"（不想要云母背景），单独勾选
    /// "WinUI 3 新设计"也应该先看到窗口圆角这个最直观的变化，而不是完全没有任何反馈。
    /// 字体（Segoe UI Variable）由 ThemeService.ApplyWinUi3Typography 负责，见该方法注释。
    /// </summary>
    public static bool CurrentWinUi3Enabled { get; private set; }

    /// <summary>
    /// 更新 WinUI 3 设计开关的当前状态并立即把窗口圆角同步到所有已打开窗口——不重新走一遍
    /// 完整的 Mica/Acrylic 背景材质切换（那部分仍然完全由 CurrentEnabled/SetEnabled 控制，
    /// 两个开关互不覆盖），只是让"圆角"这一项视觉效果响应新增的这个独立开关。
    /// 调用时机跟 SetEnabled 完全对应：App.xaml.cs 启动时用早期配置调一次，设置页
    /// Save_Click 里用户改动开关后再调一次，做到不需要重启就能立即看到效果。
    /// </summary>
    public static void SetWinUi3Enabled(bool enabled)
    {
        if (CurrentWinUi3Enabled == enabled) return;
        CurrentWinUi3Enabled = enabled;
        foreach (Window window in Application.Current.Windows)
        {
            Apply(window, CurrentEnabled);
        }
    }
    ...
    public static void Apply(Window window, bool enabled)
    {
        ...
        // 圆角由"Win11 新光效"和"WinUI 3 新设计"两个开关共同决定，任意一个开着就应该有圆角——
        // 见 CurrentWinUi3Enabled 注释，圆角是两者共有的基础视觉特征，不应该只挂在其中一个开关下。
        var cornerEnabled = enabled || CurrentWinUi3Enabled;
        var corner = cornerEnabled ? DWMWCP_ROUND : DWMWCP_DEFAULT;
        ...
        if (backdropApplied)
        {
            ...
            {
                ForceDwmResurface(hwnd);
                // 修复"同时开启 Win11 视觉效果 + 整窗全局透明时，全局透明看起来完全没效果"：
                // SettingsPage.PerformSave 特意先调 Win11EffectsService.SetEnabled 再调
                // ThemeService.ApplyGlobalWindowTransparency，就是为了让"整窗透明"是最后
                // 生效的一步（注释见 PerformSave 里"先切换 Mica/Acrylic，再最后应用整窗透明"）。
                // 但这里的 ForceDwmResurface 是通过 Dispatcher.InvokeAsync 在 Loaded 优先级
                // 异步排队执行的，实际执行时机反而落在 PerformSave 里那次同步的
                // ApplyGlobalWindowTransparency 调用之后——SetWindowPos(SWP_FRAMECHANGED)
                // 强制 DWM 重新分配/合成这块窗口的呈现表面，会把刚设置好的 WS_EX_LAYERED
                // alpha 混合结果一起冲掉，表现就是"设置里明明勾了全局透明，界面却看不出任何
                // 透明效果"。GlobalOpacityHookState 那套 WM_STYLECHANGED/WM_DWMCOMPOSITIONCHANGED
                // 钩子理论上会自愈，但有 200ms 冷却节流、且不保证消息一定按预期顺序送达；
                // 这里直接在重合成完成后同步补一次 ApplyGlobalOpacityToWindow，不依赖任何
                // 消息时序，只要用户开着整窗透明就必定在 Win11 特效重合成之后重新生效一次。
                if (ThemeService.CurrentGlobalTransparencyEnabled)
                    ThemeService.ApplyGlobalOpacityToWindow(window);
